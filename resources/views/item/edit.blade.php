...
<script type="text/javascript">
	@php
	$editting_item_name = $item->name;
	@endphp
	var item_names = [@foreach($items as $item) @if ($item->name != $editting_item_name) "{{$item->name}}", @endif @endforeach];
    ...
    <form action="/item/update/{{ $item->id }}" method="post" class="form-horizontal form-bordered" enctype="multipart/form-data">
    	@csrf
        <div class="form-group">
            ...
            <div class="col-md-7">
                <select id="example-select2" name="part" class="select-select2" style="width: 100%;" data-placeholder="Choose part" required>
                    ...
                    <option value="{{ $part->id }}" @if ($part->id == $item->part_id) selected @endif>{{$part->name}}</option>
                    @endforeach
                </select>
            </div>
        </div>
        <div class="form-group">
            ...
            <div class="col-md-7">
                <input id="item_name" type="text" name="name" class="form-control" required onchange="check_item_name()" value="{{ $item->name }}">
        ...
        <div class="form-group">
            ...
            <div class="col-md-7">
                <input type="number" name="cost" class="form-control" required value="{{ $item->cost }}">
                <span class="help-block">ex: 1200$</span>
            </div>
        </div>
        <div class="form-group text-center">
        	<img src="/img/{{ $item->image }}" height="500px">
        </div>
        <div class="form-group">
            <label class="col-md-3 control-label" for="example-file-input">Alter Image</label>
            <div class="col-md-9">
